#!/usr/bin/env python3
"""Patch cancellation-reason catalog EN targets and append missing units."""
from __future__ import annotations

import re
from pathlib import Path

from cancellation_reason_en_targets import APPEND_SOURCES, CANCELLATION_REASON_EN_BY_ID
...
def patch_existing(path: Path) -> int:
    ...
    updated = 0
    for uid, template in CANCELLATION_REASON_EN_BY_ID.items():
        pattern = re.compile(
...
def append_missing(path: Path, *, with_en_target: bool, spanish_target: bool) -> int:
    ...
    for uid, source in APPEND_SOURCES.items():
        if f'id="{uid}"' in text:
            continue
        en = CANCELLATION_REASON_EN_BY_ID[uid]
        if with_en_target:
