#!/usr/bin/env python3
"""Patch questionnaires.* targets in messages.en-US.xlf without regenerating the file."""
...
from questionnaire_en_targets import QUESTIONNAIRE_EN_BY_ID  # noqa: E402

XLF = ROOT / "src/locale/messages.en-US.xlf"
UNIT_RE = re.compile(
    r'(<trans-unit id="(questionnaires\.[^"]+)"[^>]*>\s*<source>(.*?)</source>\s*<target>)(.*?)(</target>)',
    re.S,
)
...
def main() -> int:
    ...
    def repl(match: re.Match[str]) -> str:
        ...
        template = QUESTIONNAIRE_EN_BY_ID.get(uid)
        if template is None:
            missing.append(uid)
    ...
    unused = sorted(set(QUESTIONNAIRE_EN_BY_ID) - found_ids)
    if missing:
