...
export class CatalogExperienceLevelService {
  ...
  list(countryId: number, page = 0, size = 20, search?: string | null): Observable<{ items: CatalogExperienceLevel[]; total: number }> {
    const body = {
  ...
  delete(id: number): Observable<void> {
    return this.http.delete<void>(this.api.apiUrl(`/api/v1/experience-levels/${id}`), {
      headers: this.api.buildHeaders(),
    });
  }
