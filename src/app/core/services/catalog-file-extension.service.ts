...
export class CatalogFileExtensionService {
  ...
  list(countryId: number | null, page = 0, size = 20, search?: string | null): Observable<{ items: CatalogFileExtension[]; total: number }> {
    const body = {
      search: search?.trim() || null, countryId, isActive: null, filters: [], ordersBy: ['code:asc'] as string[] };
    return this.http
