...
export class CatalogCompanyDepartmentService {
  ...
  list(catalogCompanyId: number, page = 0, size = 20, search?: string | null): Observable<{ items: CatalogCompanyDepartment[]; total: number }> {
    const body = {
  ...
  delete(id: number): Observable<void> {
    return this.http.delete<void>(this.api.apiUrl(`/api/v1/company-departments/${id}`), {
      headers: this.api.buildHeaders(),
    });
  }
