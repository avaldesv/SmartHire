...
export class CatalogCharacteristicService {
  ...
  list(countryId: number, page = 0, size = 20, search?: string | null): Observable<{ items: CatalogCharacteristic[]; total: number }> {
    const body = {
  ...
  delete(id: number): Observable<void> {
    return this.http.delete<void>(this.api.apiUrl(`/api/v1/characteristics/${id}`), {
      headers: this.api.buildHeaders(),
    });
  }
