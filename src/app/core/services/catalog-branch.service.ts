...
export class CatalogBranchService {
  ...
  list(countryId: number, page = 0, size = 20, search?: string | null): Observable<{ items: CatalogBranch[]; total: number }> {
    const body = {
  ...
  delete(id: number): Observable<void> {
    return this.http.delete<void>(this.api.apiUrl(`/api/v1/branches/${id}`), {
      headers: this.api.buildHeaders(),
    });
  }
