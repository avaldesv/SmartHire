import {
  decodeAppianChatDelimiters,
  sanitizeJobDescriptionChatMessage,
} from './sanitize-job-description-chat';
...
describe('sanitizeJobDescriptionChatMessage', () => {
  ...
  it('decode keeps blank lines before bullets; description sanitizer collapses them', () => {
    const raw = 'Lo que harás|*||*|- Uno';
    expect(decodeAppianChatDelimiters(raw)).toBe('Lo que harás\n\n- Uno');
    expect(sanitizeJobDescriptionChatMessage(raw)).toBe('Lo que harás\n- Uno');
  });
